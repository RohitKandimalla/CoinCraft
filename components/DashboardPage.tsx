...
function buildViewRoute(viewKey: string) {
  return `/dashboard/${encodeURIComponent(viewKey)}`;
}
...
export function DashboardPage({ initialViewKey }: DashboardPageProps) {
  ...
  const [selectedViewKey, setSelectedViewKey] = useState<string>(initialViewKey || 'overall');
  ...
  useEffect(() => {
    setSelectedViewKey(initialViewKey || 'overall');
  }, [initialViewKey]);
  ...
  useEffect(() => {
    ...
    if (!exists) {
      const fallbackKey = accountViews[0].key;
      setSelectedViewKey(fallbackKey);
      router.replace(buildViewRoute(fallbackKey));
    }
  }, [accountViews, selectedViewKey, router]);
  ...
  const handleSaveBaselineOverride = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/overrides/${encodeURIComponent(String(selectedView.key))}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          {lastSync && (
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">Last refreshed: {lastSync}</p>
          )}
        </div>
        <button
          onClick={handleSync}
            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  router.push(buildViewRoute(String(view.key)));
                }}
      ...
      {selectedView && (
        <div className="rounded-lg border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
          <h3 className="text-base font-semibold text-gray-900 dark:text-white">Manual Baseline Override</h3>
